...
export default function createEst({ familiares, setState }) {
  const [credentialsFam, setCredentialsFam] = useState({
    nombre: "",
    apellido: "",
    email: "",
  });

  const handleChangeFam = (e) => {
    setCredentialsFam({ ...credentialsFam, [e.target.name]: e.target.value });
  };
  const handleSubmitFam = async (e) => {
    e.preventDefault();
    const { nombre, apellido, email } = credentialsFam;
    const iniciales = obtenerIniciales(nombre);
    const data = {
      user: iniciales.toLowerCase() + apellido.toLowerCase() + "_" + "fam",
    ...
    for (let i = 0; i < familiares.length; i++) {
      ...
      if (user === data.user) {
        ...
        while (familiares.some((est) => est.user === newUser)) {
    ...
    const response = await axios.post("/api/familiar/", data);
  ...
  const emailControl3 = familiares.find(
    (fam) => fam.email == credentialsFam.email
  );
  ...
  let famcontrol;
  if (emailControl3) famcontrol = false;
  if (!emailControl3) famcontrol = true;
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Crear Usuario de Familiar</DialogTitle>
          <DialogDescription>
            Ingrese los datos del nuevo Familiar
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="sm:justify-start">
          <DialogClose asChild>
            {credentialsFam.nombre != "" &&
            credentialsFam.email != "" &&
            famcontrol &&
            credentialsFam.apellido != "" ? (
              <form
                onSubmit={(e) => handleSubmitFam(e)}
